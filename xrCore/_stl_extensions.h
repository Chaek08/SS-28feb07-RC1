...
public:
    ...
    xalloc() noexcept {}
    xalloc(const xalloc<T>&) noexcept {}
    template<class _Other>
    xalloc(const xalloc<_Other>&) noexcept {}
    template<class _Other>
    xalloc<T>& operator=(const xalloc<_Other>&) { return *this; }

    pointer allocate(size_type n, const void* = 0) {
        if (n == 0) n = 1;
        return static_cast<pointer>(xr_malloc((u32)(n * sizeof(T))));
    }
...
struct xr_allocator {
    template <typename T>
    struct helper { typedef xalloc<T> result; };
...
public:
    ...
    void clear_and_reserve() {
        if (this->capacity() <= (this->size() + this->size() / 4))
            clear_not_free();
        else {
            u32 old = size();
            clear_and_free();
    ...
    const T& operator[](typename inherited::size_type _Pos) const { { VERIFY(_Pos < this->size()); } return *(this->begin() + _Pos); }
    T&       operator[](typename inherited::size_type _Pos)       { { VERIFY(_Pos < this->size()); } return *(this->begin() + _Pos); }
};
...
public:
    ...
    bool operator<=(const xr_stack<_Ty, _C>& _X) const { return !(_X < *this); }
    bool operator>=(const xr_stack<_Ty, _C>& _X) const { return !(*this < *this); }
protected:
    _C c;
};
